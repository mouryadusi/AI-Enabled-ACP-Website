...
Normalize a model probability into a safe 0..1 range.
Handles:
undefined/null
strings returned by an API
NaN / Infinity
percentages such as 82 instead of 0.82
values outside the valid probability range
*/
...
// Support APIs that return 0..100 instead of 0..1.
const normalized =
numericValue > 1
? numericValue / 100
: numericValue;
...
useState<ConflictPrediction["model"]>("xgboost");
const [prediction, setPrediction] =
useState<ConflictPrediction | null>(null);
const [loading, setLoading] = useState(false);
const [error, setError] = useState<string | null>(null);

const flight =
mockFlights.find((f) => f.id === selectedId) ??
mockFlights[0];
...
fetchConflictPrediction(selectedId, model)
  .then((res) => {
    if (cancelled) return;
  ...
  .catch((err) => {
    ...
    console.error(
      "Failed to fetch conflict prediction:",
      err,
    );
...
Always derive a safe probability before giving it to
the UI or SVG-based RiskGauge.
*/
...
<p className="eyebrow">Interactive Prototype</p>
    ...
    <p className="mt-3 max-w-2xl text-ink-muted">
      The model predicts whether an aircraft interaction is
      likely to enter a conflict, based on observed spatial
      and kinematic relationships — not on any single
    ...
    <GlassPanel className="p-6 lg:col-span-5">
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-2 gap-4 font-mono text-xs sm:grid-cols-4">
          <Metric
            label="Altitude"
            value={`${flight.altitudeFt.toLocaleString()} ft`}
          />

          <Metric
            label="Heading"
            value={`${flight.headingDeg}°`}
          />

          <Metric
            label="Speed"
            value={`${flight.speedKts} kts`}
          />

          <Metric
            label="Nearby traffic"
            value={`${flight.nearbyTraffic} ac`}
          />
        </div>
      </div>
      ...
      <div className="mt-5 border-t border-line/60 pt-5">
        ...
        <div className="grid grid-cols-2 gap-4 font-mono text-xs sm:grid-cols-3">
          <Metric
            label="Horiz. separation"
            value={`${flight.separationNm} nm`}
          />

          <Metric
            label="Vert. separation"
            value={`${flight.verticalSeparationFt} ft`}
          />

          <Metric
            label="Relative speed"
            value={`${flight.closingRateKts} kt`}
          />

          <Metric
            label="Predicted CPA"
            value={`${flight.dcpaNm} nm`}
          />

          <Metric
            label="Time to CPA"
            value={`${flight.tcpaSeconds} s`}
          />
        </div>
      </div>
    ...
    <GlassPanel className="flex flex-col items-center justify-center p-6 lg:col-span-4">
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          >
            <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-signal-conflict">
          ...
          >
            ...
            {probability === null ? (
              <div className="mt-8 rounded-lg border border-signal-conflict/30 bg-signal-conflict/5 px-5 py-4">
                <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-signal-conflict">
                  Invalid probability
                </p>

                <p className="mt-2 text-sm text-ink-muted">
                  The prediction service returned an
